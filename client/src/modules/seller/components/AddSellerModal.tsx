...
export const AddSellerModal: React.FC<AddSellerModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleClose = () => {
    ...
    setRequireAdditional(true); setError(null);
    onClose();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (requireAdditional) {
      if (!phone.trim()) {
        setError('Phone number is required when additional fields are enabled');
        return;
      }
      if (!address.trim()) {
        setError('Address is required when additional fields are enabled');
        return;
      }
      if (!gstNumber.trim()) {
        setError('GST Number is required when additional fields are enabled');
        return;
      }
    }
  ...
  return (
    ...
    >
      <form id="add-seller-form" onSubmit={handleSubmit} className="space-y-4">
        {error && (
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              maxLength={15}
              className={`w-full bg-slate-950/80 border rounded-xl pl-10 pr-4 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-1 transition-colors ${
                requireAdditional
        ...
        <div>
          ...
          <div className="relative">
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-slate-950/80 border border-slate-800 rounded-xl pl-10 pr-4 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 resize-none"
            />
          </div>
        </div>
      </form>
